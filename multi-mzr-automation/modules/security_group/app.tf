...
resource "ibm_is_security_group_rule" "app_rule_22_bastion1" {
...
resource "ibm_is_security_group_rule" "app_rule_22_bastion2" {
  group     = ibm_is_security_group.app.id
  direction = "inbound"
...
/**
* Security Group Rule for Windows App Server with OpenSSH
* This inbound rule will allow the Bastion server to ssh connect to the Windows App server on port 22.
**/

resource "ibm_is_security_group_rule" "openssh_app_rule_22_bastion1" {
  count     = lower(var.app_os_type) == "windows" ? 1 : 0
  group     = ibm_is_security_group.app.id
  direction = "inbound"
  remote    = var.bastion1_subnet
  tcp {
...
resource "ibm_is_security_group_rule" "openssh_app_rule_22_bastion2" {
  count     = lower(var.app_os_type) == "windows" ? 1 : 0
  group     = ibm_is_security_group.app.id
  direction = "inbound"
  remote    = var.bastion2_subnet
