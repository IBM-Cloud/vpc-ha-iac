...
resource "ibm_is_security_group_rule" "web_rule_22_bastion1" {
...
resource "ibm_is_security_group_rule" "web_rule_22_bastion2" {
  group     = ibm_is_security_group.web.id
  direction = "inbound"
...
/**
* Security Group Rule for Windows Web Server with OpenSSH
* This inbound rule will allow the Bastion server to ssh connect to the Windows Web server on port 22.
**/

resource "ibm_is_security_group_rule" "openssh_web_rule_22_bastion1" {
  count     = lower(var.web_os_type) == "windows" ? 1 : 0
  group     = ibm_is_security_group.web.id
  direction = "inbound"
  remote    = var.bastion1_subnet
  tcp {
...
resource "ibm_is_security_group_rule" "openssh_web_rule_22_bastion2" {
  count     = lower(var.web_os_type) == "windows" ? 1 : 0
  group     = ibm_is_security_group.web.id
  direction = "inbound"
  remote    = var.bastion2_subnet
