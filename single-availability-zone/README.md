...
This code creates a resilient infrastructure for a given SZR.  The VSIs are placed in instance groups (also known as auto-scale) to provide an automated way to add or remove VSIs based on load conditions. Currently, the placement group is not part of this code, the IBM VSI scheduler attempts (best effort) to place the VSIs on a different host.

**Note**: Placement group will be added in the following code release.
